...
//TODO: Toooooo ugly!!!!
function generateQueryWhereClause(pod_name, show_live, start_time, end_time, customClause = "") {
    var whereClause = " WHERE pod_name = '\"" + pod_name + "\"'" + customClause;

    var orderByClause = " ORDER BY time desc";
    var limitClause = "";

    if ((!start_time && !end_time) || show_live) {
...
function Resourceusage(Resourceusage) {
    ...
    Resourceusage.getAllResourceUsages = async function (pod_name, show_live, start_time, end_time) {
        ...
        var whereClause = generateQueryWhereClause(pod_name, show_live, start_time, end_time, " AND container_name='\"\"'");
        var whereClauseWithoutAnyContainerName = generateQueryWhereClause(pod_name, show_live, start_time, end_time);
